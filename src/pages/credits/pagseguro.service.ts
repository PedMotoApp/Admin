...
export class PagSeguroService {
  ...
  constructor(private http: Http, 
    ...
    public httpd: HttpClient) {
      ...
      this.apiKey = 'AIzaSyC22g7vHTquiAZZIJ7GlmCRyI_p-j5ccdU'    
  }

  public setEmail(email: string) {    
    this.pagSeguroEmail = email;
  }

  public setToken(token: string) {
    this.pagSeguroToken = token;
  }
  ...
  public startSession(): Promise<any> {
    ...
    let myData = JSON.stringify({ pagSeguroEmail: this.pagSeguroEmail, pagSeguroToken: this.pagSeguroToken});
    const headers = new HttpHeaders({'Content-Type': 'application/json'});
    
    return this.httpd.post(this.options.remoteApi.sessionURL, myData, {headers: headers}).toPromise()
  }

  /**
   * Recupera as {{dataText.options}} de pagamento.
  ...
  public addCheckoutData(data: PagSeguroData, skipPatchForm?: boolean) {
    ...
    if (!skipPatchForm) {
      ...
      if (this.checkoutData && this.checkoutData.sender && this.paymentForm) {
        ...
        if (this.checkoutData.sender.name &&
          this.paymentForm.value.card &&
          !this.paymentForm.value.card.name) {
        ...
        if (this.checkoutData.sender.documents &&
          this.checkoutData.sender.documents.document.type === 'CPF'
          && this.paymentForm.value.card && !this.paymentForm.value.card.cpf) {
          this.paymentForm.patchValue({
            card: {
  ...
  buildPagSeguroData(): PagSeguroData {
    let data: PagSeguroData = {
      ...
      pagSeguroToken: this.pagSeguroToken,
      ...
      sender: {
        phone: {
          ...
          number: this.dataInfo.userInfo.tel.substring(2)
        },
        documents: {
          document: {
            type: 'CPF',
   ...
   * Ela irá passar os dados resgatados, para uma Firebase Function, que irá concluir o processo
   *
   * @param data
   */
  public checkout(): Promise<any> {
    ...
    data.pagSeguroEmail = this.pagSeguroEmail;
    data.pagSeguroToken = this.pagSeguroToken;
